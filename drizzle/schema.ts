...
export const users = mysqlTable("users", {
  ...
  ageGroup: mysqlEnum("age_group", ["10s", "20s", "30s", "40s", "50s"]), // 연령대 (10대, 20대, 30대, 40대, 50대 이상)
  gender: mysqlEnum("gender", ["male", "female", "other"]), // 성별 (남성, 여성, 선택 안 함)
...
export const stores = mysqlTable("stores", {
  id: int("id").autoincrement().primaryKey(),
  ownerId: int("owner_id").notNull(), // users.id 참조
  name: varchar("name", { length: 255 }).notNull(),
  category: mysqlEnum("category", ["cafe", "restaurant", "beauty", "hospital", "fitness", "other"]).notNull(), // 카페, 음식점, 뷰티, 병원, 헬스장, 기타
  ...
  rating: decimal("rating", { precision: 2, scale: 1 }).default("0.0"), // 별점 (1.0~5.0, 관리자 수동 조정 가능)
  ratingCount: int("rating_count").default(0), // 별점 개수 (관리자 수동 조정 가능)
...
export const coupons = mysqlTable("coupons", {
  id: int("id").autoincrement().primaryKey(),
  storeId: int("store_id").notNull().references(() => stores.id, { onDelete: 'cascade' }), // stores.id 참조
  title: varchar("title", { length: 255 }).notNull(), // 쿠폰 제목
  description: text("description"), // 쿠폰 설명
  discountType: mysqlEnum("discount_type", ["percentage", "fixed", "freebie"]).notNull(), // 할인 유형
  discountValue: int("discount_value").notNull(), // 할인 값 (% 또는 원)
  minPurchase: int("min_purchase").default(0), // 최소 구매 금액
  maxDiscount: int("max_discount"), // 최대 할인 금액
  totalQuantity: int("total_quantity").notNull(), // 총 발행 수량
  remainingQuantity: int("remaining_quantity").notNull(), // 남은 수량
...
export const userCoupons = mysqlTable("user_coupons", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }), // users.id 참조
  couponId: int("coupon_id").notNull().references(() => coupons.id, { onDelete: 'cascade' }), // coupons.id 참조
...
export const couponUsage = mysqlTable("coupon_usage", {
  id: int("id").autoincrement().primaryKey(),
  userCouponId: int("user_coupon_id").notNull(), // user_coupons.id 참조
  storeId: int("store_id").notNull(), // stores.id 참조
  userId: int("user_id").notNull(), // users.id 참조
  verifiedBy: int("verified_by").notNull(), // 검증한 사장님 users.id
...
export const userStats = mysqlTable("user_stats", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id").notNull().unique(), // users.id 참조
  points: int("points").default(0).notNull(), // 포인트
  level: int("level").default(1).notNull(), // 레벨 (1=브론즈, 2=실버, 3=골드, 4=다이아)
  totalCouponsDownloaded: int("total_coupons_downloaded").default(0).notNull(),
  totalCouponsUsed: int("total_coupons_used").default(0).notNull(),
  consecutiveCheckIns: int("consecutive_check_ins").default(0).notNull(), // 연속 출석일
  lastCheckInDate: timestamp("last_check_in_date"),
  totalCheckIns: int("total_check_ins").default(0).notNull(),
  referralCode: varchar("referral_code", { length: 20 }).unique(), // 초대 코드
  referredBy: int("referred_by"), // 추천인 users.id
  totalReferrals: int("total_referrals").default(0).notNull(), // 추천한 사람 수
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
});
...
export const badges = mysqlTable("badges", {
  ...
  points: int("points").default(0).notNull(), // 획득 시 포인트
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const userBadges = mysqlTable("user_badges", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id").notNull(), // users.id 참조
  badgeId: int("badge_id").notNull(), // badges.id 참조
...
export const checkIns = mysqlTable("check_ins", {
  ...
  points: int("points").default(10).notNull(), // 획득 포인트
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const reviews = mysqlTable("reviews", {
  ...
  storeId: int("store_id").notNull(), // stores.id 참조
  userId: int("user_id").notNull(), // users.id 참조
  userCouponId: int("user_coupon_id"), // user_coupons.id 참조 (쿠폰 사용 후 리뷰)
  rating: int("rating").notNull(), // 1-5점
...
export const visits = mysqlTable("visits", {
  id: int("id").autoincrement().primaryKey(),
  storeId: int("store_id").notNull(), // stores.id 참조
  userId: int("user_id"), // users.id 참조 (선택적, 비로그인 사용자도 기록)
...
export const searchLogs = mysqlTable("search_logs", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id"), // users.id 참조 (선택적)
...
export const adTransactions = mysqlTable("ad_transactions", {
  id: int("id").autoincrement().primaryKey(),
  storeId: int("store_id").notNull(), // stores.id 참조
  visitId: int("visit_id").notNull(), // visits.id 참조
  amount: int("amount").notNull(), // 광고비 (센트 단위)
...
export const userMissions = mysqlTable("user_missions", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id").notNull(), // users.id 참조
  missionId: int("mission_id").notNull(), // missions.id 참조
  progress: int("progress").default(0).notNull(), // 현재 진행도
...
export const pointTransactions = mysqlTable("point_transactions", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id").notNull(), // users.id 참조
  amount: int("amount").notNull(), // 포인트 변동량 (+ 또는 -)
  type: mysqlEnum("type", ["earn", "spend", "bonus", "mission", "referral"]).notNull(),
  description: text("description"),
  relatedId: int("related_id"), // 관련 ID (쿠폰 ID, 미션 ID 등)
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const notifications = mysqlTable("notifications", {
  ...
  type: mysqlEnum("type", ["coupon_expiring", "new_coupon", "nearby_store", "mission_complete", "level_up", "general"]).notNull(),
  isRead: boolean("is_read").default(false).notNull(),
  relatedId: int("related_id"), // 관련 ID (쿠폰 ID, 가게 ID 등)
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const emailLogs = mysqlTable("email_logs", {
  ...
  type: mysqlEnum("type", ["new_coupon", "expiry_reminder"]).notNull(),
...
export const appVersions = mysqlTable("app_versions", {
  ...
  updateMode: mysqlEnum("update_mode", ["none", "soft", "hard"]).default("none").notNull(), // 업데이트 모드
 ...
 */
export const installFunnelEvents = mysqlTable("install_funnel_events", {
  id: int("id").autoincrement().primaryKey(),
  sessionId: varchar("session_id", { length: 255 }).notNull(), // 세션 ID (UUID)
  userId: int("user_id"), // users.id 참조 (선택적, 로그인 전에는 null)
 ...
 */
export const emergencyBanners = mysqlTable("emergency_banners", {
  id: int("id").autoincrement().primaryKey(),
  title: varchar("title", { length: 255 }).notNull(),
  content: text("content").notNull(),
  type: mysqlEnum("type", ["info", "warning", "error", "maintenance"]).default("info").notNull(),
  priority: int("priority").default(0).notNull(), // 높을수록 우선순위 높음
 ...
 */
export const bannerInteractions = mysqlTable("banner_interactions", {
  id: int("id").autoincrement().primaryKey(),
  bannerId: int("banner_id").notNull().references(() => emergencyBanners.id, { onDelete: 'cascade' }),
  userId: int("user_id"), // users.id 참조 (선택적)
  sessionId: varchar("session_id", { length: 255 }).notNull(),
  interactionType: mysqlEnum("interaction_type", ["view", "click", "dismiss"]).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const clientErrors = mysqlTable("client_errors", {
  ...
  sessionId: varchar("session_id", { length: 255 }).notNull(),
  appVersion: varchar("app_version", { length: 20 }).notNull(),
  errorType: mysqlEnum("error_type", ["js_error", "promise_rejection", "api_failure", "network_error"]).notNull(),
...
export const featureFlags = mysqlTable("feature_flags", {
  ...
  rolloutPercentage: int("rollout_percentage").default(0).notNull(), // 0-100
...
export const userFeatureFlags = mysqlTable("user_feature_flags", {
  ...
  flagId: int("flag_id").notNull().references(() => featureFlags.id, { onDelete: 'cascade' }),
 ...
 */
export const versionStats = mysqlTable("version_stats", {
  id: int("id").autoincrement().primaryKey(),
  appVersion: varchar("app_version", { length: 20 }).notNull(),
