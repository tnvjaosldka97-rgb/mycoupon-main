...
export default function Home() {
  ...
  const storesQuery = trpc.stores.list.useQuery({ limit: 50 });
  ...
  // 사용자 위치 가져오기 (GPS 권한 요청) - 최적화: 기본 위치 먼저 표시
  useEffect(() => {
    // 기본 위치를 먼저 설정하여 지도가 즉시 표시되도록 함
    const defaultLocation = { lat: 37.5665, lng: 126.9780 };
    setUserLocation(defaultLocation);
    console.log('📍 기본 위치 설정 (서울 명동):', defaultLocation);
    
    // GPS 권한 요청 및 위치 가져오기 (백그라운드에서 처리)
    if (navigator.geolocation) {
      console.log('📍 GPS 위치 정보 요청 시작 - 브라우저에서 위치 권한 팝업이 표시됩니다');
      
      // 타임아웃 설정 (5초로 단축 - 더 빠른 반응)
      const options = {
        enableHighAccuracy: false, // 정확도보다 속도 우선
        timeout: 5000, // 5초 타임아웃
        maximumAge: 60000, // 1분 캐시 사용 (빠른 응답)
      };
      
      // getCurrentPosition 호출 시 브라우저에서 자동으로 "위치 정보 권한을 허용하시겠습니까?" 팝업 표시
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          console.log('📍 GPS 위치 정보 가져오기 성공:', location);
          toast.success('현재 위치를 가져왔습니다!');
        },
        (error) => {
          console.warn('📍 GPS 위치 정보 가져오기 실패, 기본 위치 사용:', error);
          // 에러가 발생해도 기본 위치는 이미 설정되어 있으므로 조용히 처리
          if (error.code === error.PERMISSION_DENIED) {
            toast.warning('위치 권한이 거부되었습니다. 기본 위치를 사용합니다.');
          } else if (error.code === error.TIMEOUT) {
            toast.warning('위치 정보 요청 시간이 초과되었습니다. 기본 위치를 사용합니다.');
          }
          // 기본 위치는 이미 설정되어 있음
        },
        options
      ...
      // GPS를 지원하지 않는 경우 기본 위치는 이미 설정되어 있음
      console.warn('📍 GPS를 지원하지 않음, 기본 위치 사용');
    }
  }, []);
  ...
  const handleDownloadCoupon = async (couponId: number) => {
    if (!user) {
      ...
      if (confirm('쿠폰을 다운로드하려면 로그인이 필요합니다. 로그인 하시겠습니까?')) {
    ...
    try {
      ...
      alert(error.message || '쿠폰 다운로드에 실패했습니다.');
    }
  };
                        ...
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              className="rounded-xl bg-gradient-to-r from-primary to-accent flex-shrink-0"
                              disabled={downloadCoupon.isPending}
                              size="sm"
                            >
